...
**管理方式**：
- 管理员在后台维护违禁词列表
- 每行一个词，服务端统一去重、转小写并限制单词长度和总数量
- 修改黑名单会写入管理员审计日志

**拦截方式**：
- 明文消息在 WebSocket 写库前由服务端拦截
- 端到端加密消息在客户端加密前做本地检查
- 服务端无法解密端到端加密内容，因此不能对密文做强制内容审查

**安全边界**：
- 黑名单用于基础内容治理，不替代人工审核
- 对抗性规避（空格、同音字、图片文字）需要后续更复杂的审核机制
...
**文本消息加密**：
- 用户可为每个会话设置本地加密口令
- 发送前在浏览器内使用 PBKDF2-SHA256 派生密钥，并用 AES-GCM 加密文本内容
- 服务端、数据库、WebSocket Durable Object 和管理员消息搜索只处理密文信封
- 未设置口令的旧消息继续按明文兼容显示

**密钥边界**：
- 口令只保存在当前浏览器本地，不上传到服务端
- 不同会话使用不同的附加认证数据，防止密文被跨会话搬移后正常解密
- 口令遗失后服务端无法恢复加密消息明文

**当前范围**：
- 当前版本加密文本消息内容
- 附件文件仍走现有 R2 上传、文件类型限制和发送者 key 归属校验
- 服务端无法读取密文内容，因此违禁词拦截只能强制覆盖明文消息
...
- update_blocked_words：管理员更新违禁词黑名单
- 其他管理员操作：根据需要扩展
...
- [ ] HTTPS 已启用
- [ ] CORS 白名单已配置
- [ ] API 安全响应头已启用（nosniff、no-referrer、DENY frame、权限策略）
- [ ] 数据库已初始化且备份存在
- [ ] 文件上传验证已启用
- [ ] 审计日志表已创建
- [ ] WebSocket 速率限制已启用
